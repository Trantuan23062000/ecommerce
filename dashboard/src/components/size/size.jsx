...
import { getSizes,SearchSize,DeletSize } from "../../api/size";
import DeteteModal from "../size/DeteteModal";
import ModalCreate from "../size/ModalCreate";
...
const Brand = () => {
  ...
  const fetch = async () => {
    if (key !== "") {
      try {
        const res = await SearchSize(key);
        setData(res.data.DT);
      } catch (error) {
        console.log(error);
      }
  ...
  const cofirmModalDelete = async () => {
    let response = await DeletSize(edit.id);
  ...
  const fetchdata = async () => {
    let response = await getSizes(currentPage, currentLimit);
    if (response && response.data && response.data.EC === 0) {
      setTotalPages(response.data.DT.totalPages);
      setData(response.data.DT.size);
    }
  };
